...
from lerobot.envs.utils import check_env_attributes_and_types, close_envs
    ...
    *,
    ...
    while not np.all(done) and step < max_steps:
        ...
        for chunk_step in range(chunk_exec_steps):
            ...
            _, reward, terminated, truncated, info = env.step(action_step)
            if render_callback is not None:
                render_callback(env)
...
def main() -> None:
    ...
    _, env_postprocessor = make_env_pre_post_processors(env_cfg=env_cfg, policy_cfg=policy_cfg)
