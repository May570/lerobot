...
_LATENCY_RESIDUAL_BALL_POS_RAW = "__latency_residual_ball_pos_raw"


@dataclass
class LatencyResidualRuntimeConfig:
...
@dataclass
class EvalPipelineWithLatencyResidualConfig(EvalPipelineConfig):
    residual: LatencyResidualRuntimeConfig = field(default_factory=LatencyResidualRuntimeConfig)
...
def _extract_latency_residual_state(obs: dict[str, Tensor]) -> Tensor:
    state = obs.get(OBS_STATE_RAW, obs.get(OBS_STATE))
    if state is None:
        raise KeyError(f"Missing `{OBS_STATE}` / `{OBS_STATE_RAW}` for latency residual runtime.")
    return state


def _extract_latency_residual_ball(obs: dict[str, Tensor]) -> Tensor | None:
    ball = obs.get(_LATENCY_RESIDUAL_BALL_POS_RAW)
    if ball is not None:
        return ball
    ball = obs.get("observation.ball_pos")
    if isinstance(ball, Tensor):
        return ball
    return None


def _stack_history_pair(old_value: Tensor, new_value: Tensor) -> Tensor:
    return torch.stack([old_value, new_value], dim=1)
...
def rollout(
    ...
    residual_model: LatencyResidualMLP | None = None,
    residual_model_cfg: LatencyResidualModelConfig | None = None,
    residual_runtime_cfg: LatencyResidualRuntimeConfig | None = None,
    ...
    residual_needs_ball_pos = (
        residual_model_cfg is not None and int(getattr(residual_model_cfg, "ball_dim", 0)) > 0
    )
    if not need_scene_ball_pos and isinstance(input_features, dict):
        need_scene_ball_pos = future_ball_pos_key in input_features
    need_ball_pos_runtime = need_scene_ball_pos or residual_needs_ball_pos
    ball_pos_dim = 3
    ...
    elif residual_needs_ball_pos:
        ball_pos_dim = int(residual_model_cfg.ball_dim)

    logging.info(
        "[realtime3][obs-cond] mode=%s need_scene_ball_pos=%d residual_needs_ball_pos=%d future_ball_pos_key=%s ball_pos_dim=%d",
        future_mode,
        int(need_scene_ball_pos),
        int(residual_needs_ball_pos),
        future_ball_pos_key,
        ball_pos_dim,
    )
    def _maybe_inject_ball_pos(raw_observation: dict[str, np.ndarray]) -> dict[str, np.ndarray]:
        if not need_ball_pos_runtime:
    ...
    def _prepare_policy_observation(raw_observation: dict[str, np.ndarray], step_idx: int) -> tuple[dict, dict]:
        ...
        if need_ball_pos_runtime and "observation.ball_pos" not in base_obs:
            raise RuntimeError(
                "[realtime3][ball-pos] runtime requires `observation.ball_pos` after preprocessing; "
                "aborting evaluation."
            )
        ...
        if "observation.ball_pos" in base_obs:
            ball_pos_raw = base_obs["observation.ball_pos"]
            if isinstance(ball_pos_raw, Tensor):
                if OBS_STATE in policy_obs:
                    ball_pos_raw = ball_pos_raw.to(
                        device=policy_obs[OBS_STATE].device,
                        dtype=policy_obs[OBS_STATE].dtype,
                    )
                policy_obs[_LATENCY_RESIDUAL_BALL_POS_RAW] = ball_pos_raw.detach().clone()
    ...
    def _to_policy_input(batch: dict[str, Tensor]) -> dict[str, Tensor]:
        batch_for_policy = {
            ...
            if k != ACTION and k != _LATENCY_RESIDUAL_BALL_POS_RAW
        }
    ...
    def _maybe_apply_latency_residual(
        action_item: dict[str, Any],
        latest_obs_pair: list[tuple[int, dict[str, Tensor]]],
        pending_items: list[dict[str, Any]],
    ) -> Tensor:
        action_cpu = action_item["action"]
        if (
            residual_model is None
            or residual_model_cfg is None
            or residual_runtime_cfg is None
            or not residual_runtime_cfg.enabled
        ):
            return action_cpu

        if len(latest_obs_pair) < 2:
            return action_cpu

        anchor_old_obs = action_item["anchor_old_obs"]
        anchor_new_obs = action_item["anchor_new_obs"]
        _, current_old_obs = latest_obs_pair[0]
        _, current_new_obs = latest_obs_pair[-1]

        anchor_state_history = _stack_history_pair(
            _extract_latency_residual_state(anchor_old_obs),
            _extract_latency_residual_state(anchor_new_obs),
        )
        current_state_history = _stack_history_pair(
            _extract_latency_residual_state(current_old_obs),
            _extract_latency_residual_state(current_new_obs),
        )
        base_action = action_cpu.to(dtype=torch.float32)
        batch_size = int(base_action.shape[0])

        if base_action.shape[-1] != int(residual_model_cfg.action_dim):
            msg = (
                "[latency-residual] action dim mismatch: "
                f"runtime={base_action.shape[-1]} expected={residual_model_cfg.action_dim}"
            )
            if residual_runtime_cfg.strict_shapes:
                raise RuntimeError(msg)
            logging.warning(msg)
            return action_cpu

        if anchor_state_history.shape[-1] != int(residual_model_cfg.state_dim):
            msg = (
                "[latency-residual] state dim mismatch: "
                f"runtime={anchor_state_history.shape[-1]} expected={residual_model_cfg.state_dim}"
            )
            if residual_runtime_cfg.strict_shapes:
                raise RuntimeError(msg)
            logging.warning(msg)
        ...
        delay_steps_value = int(action_item["delay_steps"])
        if delay_steps_value > int(residual_model_cfg.max_delay_value):
            logging.warning(
                "[latency-residual] clipping delay_steps from %d to %d.",
                delay_steps_value,
                int(residual_model_cfg.max_delay_value),
            )
            delay_steps_value = int(residual_model_cfg.max_delay_value)

        step_offset_value = int(action_item["step_offset"])
        if step_offset_value > int(residual_model_cfg.max_step_offset):
            logging.warning(
                "[latency-residual] clipping step_offset from %d to %d.",
                step_offset_value,
                int(residual_model_cfg.max_step_offset),
            )
            step_offset_value = int(residual_model_cfg.max_step_offset)

        residual_batch: dict[str, Tensor] = {
            "anchor_state_history": anchor_state_history,
            "current_state_history": current_state_history,
            "base_action": base_action,
            "delay_steps": torch.full((batch_size,), delay_steps_value, dtype=torch.long),
            "step_offset": torch.full((batch_size,), step_offset_value, dtype=torch.long),
        }

        if int(residual_model_cfg.ball_dim) > 0:
            anchor_old_ball = _extract_latency_residual_ball(anchor_old_obs)
            anchor_new_ball = _extract_latency_residual_ball(anchor_new_obs)
            current_old_ball = _extract_latency_residual_ball(current_old_obs)
            current_new_ball = _extract_latency_residual_ball(current_new_obs)
            if any(
                ball is None
                for ball in (anchor_old_ball, anchor_new_ball, current_old_ball, current_new_ball)
            ):
                msg = "[latency-residual] missing raw ball_pos for a ball-aware residual model."
                if residual_runtime_cfg.strict_shapes:
                    raise RuntimeError(msg)
                logging.warning(msg)
                return action_cpu
            assert anchor_old_ball is not None
            assert anchor_new_ball is not None
            assert current_old_ball is not None
            assert current_new_ball is not None
            if anchor_old_ball.shape[-1] != int(residual_model_cfg.ball_dim):
                msg = (
                    "[latency-residual] ball dim mismatch: "
                    f"runtime={anchor_old_ball.shape[-1]} expected={residual_model_cfg.ball_dim}"
                )
                if residual_runtime_cfg.strict_shapes:
                    raise RuntimeError(msg)
                logging.warning(msg)
                return action_cpu
            residual_batch["anchor_ball_history"] = _stack_history_pair(anchor_old_ball, anchor_new_ball)
            residual_batch["current_ball_history"] = _stack_history_pair(current_old_ball, current_new_ball)

        if int(residual_model_cfg.local_plan_horizon) > 0:
            plan_actions = [action_cpu]
            for pending_item in pending_items[: max(0, int(residual_model_cfg.local_plan_horizon) - 1)]:
                plan_actions.append(pending_item["action"])
            while len(plan_actions) < int(residual_model_cfg.local_plan_horizon):
                plan_actions.append(plan_actions[-1])
            residual_batch["local_plan"] = torch.stack(plan_actions, dim=1).to(dtype=torch.float32)

        corrected = residual_model.predict_corrected_action(residual_batch)["corrected_action"]
        return corrected.detach().to("cpu", dtype=action_cpu.dtype)
                    ...
                    kept_actions = [
                        {
                            ...
                            "anchor_old_obs": old_obs,
                            "anchor_new_obs": new_obs,
                            "delay_steps": int(drop_n),
                            "step_offset": int(chunk_step - drop_n),
                            "chunk_index": int(chunk_step),
                        }
                        for chunk_step, action_tensor in enumerate(chunk_actions_cpu[drop_n:], start=drop_n)
                    ]
                    action_queue.clear()
    ...
    try:
        while not np.all(done) and step < max_steps:
            ...
            if supports_background_chunking:
                ...
                action = _maybe_apply_latency_residual(
                    action_item,
                    list(obs_queue),
                    list(action_queue),
                )
...
def eval_policy(
    ...
    for batch_ix in progbar:
        ...
        rollout_data = rollout(
            ...
            residual_model_cfg=residual_model_cfg,
            residual_runtime_cfg=residual_runtime_cfg,
...
@parser.wrap()
def eval_main(cfg: EvalPipelineWithLatencyResidualConfig):
    ...
    residual_model = None
    residual_model_cfg = None
    if (
        getattr(cfg, "residual", None) is not None
        and cfg.residual.enabled
        and cfg.residual.checkpoint_dir
    ):
        logging.info("Loading latency residual checkpoint from %s", cfg.residual.checkpoint_dir)
        residual_model, residual_model_cfg = load_latency_residual_checkpoint(
            cfg.residual.checkpoint_dir,
            device=str(device),
        )
        residual_model.eval()
        logging.info(
            "[latency-residual] loaded checkpoint action_dim=%d state_dim=%d ball_dim=%d residual_scale=%.4f",
            int(residual_model_cfg.action_dim),
            int(residual_model_cfg.state_dim),
            int(residual_model_cfg.ball_dim),
            float(residual_model_cfg.residual_scale),
        )
    else:
        logging.info("[latency-residual] disabled for this eval run.")
    ...
    with torch.no_grad(), torch.autocast(device_type=device.type) if cfg.policy.use_amp else nullcontext():
        info = eval_policy_all(
            ...
            residual_model=residual_model,
            residual_model_cfg=residual_model_cfg,
            residual_runtime_cfg=cfg.residual,
    ...
    # Save info
    info["latency_residual"] = {
        "enabled": bool(getattr(cfg, "residual", None) is not None and cfg.residual.enabled),
        "checkpoint_dir": None if getattr(cfg, "residual", None) is None else cfg.residual.checkpoint_dir,
        "strict_shapes": True if getattr(cfg, "residual", None) is None else bool(cfg.residual.strict_shapes),
    }
