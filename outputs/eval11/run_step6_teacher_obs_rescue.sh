...
BRANCH_MAX_CHUNKS="${BRANCH_MAX_CHUNKS:-6}"
OUT_DIR="${OUT_DIR:-${SCRIPT_DIR}/step6_teacher_obs_rescue_chunk${TARGET_CHUNK}_$(date '+%Y%m%d_%H%M%S')}"
...
python "${EVAL_SCRIPT}" \
  ...
  --eval.batch_size=1 \
  --probe.target_chunk="${TARGET_CHUNK}" \
  --probe.branch_max_chunks="${BRANCH_MAX_CHUNKS}" \
