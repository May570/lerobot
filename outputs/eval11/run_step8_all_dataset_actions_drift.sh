...
OUT_DIR="${OUT_DIR:-${SCRIPT_DIR}/step8_all_dataset_actions_drift_$(date '+%Y%m%d_%H%M%S')}"
...
python "${EVAL_SCRIPT}" \
  ...
  --schedule.source_before=dataset \
  --schedule.source_after=dataset \
  --schedule.switch_after_chunk=0 \
