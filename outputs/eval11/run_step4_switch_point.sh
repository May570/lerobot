...
EVAL_SCRIPT="${EVAL_SCRIPT:-${PROJECT_ROOT}/src/lerobot/scripts/lerobot_eval_dyn_mini_obs_validation.py}"
...
SOURCE_BEFORE="${SOURCE_BEFORE:-online}"
SOURCE_AFTER="${SOURCE_AFTER:-dataset}"
SWITCH_AFTER_CHUNK="${SWITCH_AFTER_CHUNK:-1}"
OUT_DIR="${OUT_DIR:-${SCRIPT_DIR}/step4_switch_${SOURCE_BEFORE}_to_${SOURCE_AFTER}_after${SWITCH_AFTER_CHUNK}_$(date '+%Y%m%d_%H%M%S')}"
...
python "${EVAL_SCRIPT}" \
  ...
  --probe.max_chunks="${MAX_CHUNKS}" \
  --schedule.source_before="${SOURCE_BEFORE}" \
  --schedule.source_after="${SOURCE_AFTER}" \
  --schedule.switch_after_chunk="${SWITCH_AFTER_CHUNK}" \
  --rollout.execute_n_action_steps=4 \
  --render.save_videos=true \
  --render.max_episodes="${N_EPISODES}" \
  --output_dir="${OUT_DIR}" \
