...
def build_copied_video_name(run_name: str, episode_name: str) -> str:
    src = Path(episode_name)
    stem = src.stem
    if stem.startswith("eval_"):
        stem = stem[len("eval_") :]
    return f"{stem}_{run_name}{src.suffix}"
...
def copy_episode_videos(
    ...
    for row in episode_rows:
        ...
        if not src.exists():
            continue
        dst = dest_dir / build_copied_video_name(run_name, row["episode_name"])
