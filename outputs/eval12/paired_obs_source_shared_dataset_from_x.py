...
def make_output_dir(raw_output_dir: str | None, *, switch_inference_1based: int) -> Path:
    if raw_output_dir:
        path = Path(raw_output_dir)
    else:
        stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        path = Path("outputs/eval12") / f"shared_dataset_from_inf{int(switch_inference_1based)}_{stamp}"
...
def main() -> None:
    ...
    run_meta = {
        "mode": "paired_obs_source_shared_dataset_from_x",
