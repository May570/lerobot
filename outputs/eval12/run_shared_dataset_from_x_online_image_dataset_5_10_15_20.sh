...
EVAL_SCRIPT="${SCRIPT_DIR}/paired_obs_source_shared_dataset_from_x_online_image_from_dataset.py"
...
for X in 10 15 20; do
  OUT_DIR="${SCRIPT_DIR}/shared_dataset_from_inf${X}_online_image_dataset"
  mkdir -p "${OUT_DIR}"
  ...
  python "${EVAL_SCRIPT}" \
    ...
    --rollout.online_image_source=dataset \
    --output_dir="${OUT_DIR}" \
