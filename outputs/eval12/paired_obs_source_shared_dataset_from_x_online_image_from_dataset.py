...
2. Before inference `x`, the online lane uses its own online environment observations,
   but its images can be taken from the dataset.
    ...
    *,
    ...
    if raw_output_dir:
        ...
        path = Path("outputs/eval12") / (
            f"shared_dataset_from_inf{int(switch_inference_1based)}_online_image_{online_image_source}_{stamp}"
        )
...
def rollout_paired_episode(
    *,
    ...
    if online_image_source not in {"online", "dataset"}:
        raise ValueError(f"Unsupported online_image_source={online_image_source!r}.")
    ...
    while step < max_steps:
        ...
        if shared_input_active:
            ...
            if online_image_source == "dataset":
                for key, value in dataset_lane_batch.items():
                    if key.startswith("observation.images") and key in online_lane_frame and isinstance(value, torch.Tensor):
                        online_lane_frame[key] = value[:, -1].detach().clone() if value.ndim >= 5 else value.detach().clone()
        ...
        per_inference_records.append(
            {
                ...
                "online_image_source": str(online_image_source),
...
def build_pair_record(
    *,
    ...
    record = {
        ...
        "online_image_source": rollout_data["online_image_source"],
    ...
    record["online_image_source"] = rollout_data["online_image_source"]
    return record
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--rollout.online_image_source",
        dest="rollout_online_image_source",
        choices=["online", "dataset"],
        default="dataset",
        help="Source for the online-lane images before shared input begins.",
    )
    parser.add_argument(
        "--dataset.repo_id",
...
def main() -> None:
    ...
    if args.rollout_online_image_source not in {"online", "dataset"}:
        raise ValueError(f"Unsupported rollout.online_image_source={args.rollout_online_image_source!r}.")
    ...
    output_dir = make_output_dir(
        ...
        online_image_source=str(args.rollout_online_image_source),
    )
    ...
    run_meta = {
        "mode": "paired_obs_source_shared_dataset_from_x_online_image_from_dataset",
