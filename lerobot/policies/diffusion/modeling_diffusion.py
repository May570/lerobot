...
class DiffusionPolicy(PreTrainedPolicy):
    ...
    def select_action(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        This method handles caching a history of observations and an action tr-n_leftajectory generated by the
...
class DiffusionModel(nn.Module):
    def __init__(self, config: DiffusionConfig):
        ...
        self._rtc_prev_chunk = None  # 用于 RTC 引导

    # ========= inference  ============
    def guided_sample(
        self, batch_size: int, global_cond: Tensor, prev_chunk: Tensor, generator: torch.Generator | None = None
    ) -> Tensor:
        ...
        delay = self.config.rtc_delay  # 7
        soft_steps = self.config.rtc_soft_steps  # 3
        beta = self.config.rtc_beta # 2.0
        
        # logging.info(f"0. prev_chunk: {prev_chunk[1]}")
        x_cut = prev_chunk[:, delay:, :]
        x_expanded = torch.nn.functional.pad(x_cut, (0, 0, 0, H - x_cut.shape[1])) 
        # logging.info(f"1. x_expanded: {x_expanded[1]}")

        guidance_weights = torch.zeros(H, device=device)
        guidance_weights[:delay] = 1
        for i in range(delay, min(delay + soft_steps, H)):
            guidance_weights[i] = torch.exp(torch.tensor(-float(i - delay) / float(soft_steps)))
        # logging.info(f"1.5. guidance_weights: {guidance_weights}")
        guidance_weights = guidance_weights.view(1, H, 1).expand(B, H, D)
        # logging.info(f"2. guidance_weights: {guidance_weights[1]}")
        ...
        for t in self.noise_scheduler.timesteps:
            ...
            # 推测最终动作 A_hat = sample + (1 - τ) * v
            tau = t.float() / self.noise_scheduler.config.num_train_timesteps
            A_hat = sample + (1 - tau) * v

            loss = ((A_hat - x_expanded) * guidance_weights).pow(2).mean()
            # logging.info(f"3. loss: {loss}")
            grad = torch.autograd.grad(loss, sample, retain_graph=False, create_graph=False)[0]

            # guidance 权重调节
            r2 = ((1 - tau) ** 2) / (tau ** 2 + (1 - tau) ** 2 + 1e-8)
            weight = min(beta, ((1 - tau) / tau * r2).item())
            guidance = weight * grad

            with torch.no_grad():
                pred = v + guidance
                sample = self.noise_scheduler.step(pred, t, sample, generator=generator).prev_sample
